...
{
    double manhattanDistance = abs(cx - x) + abs(cy - y);

    if (manhattanDistance >= ((double) rad)) {
        return fc2;
    }
    ...
    double longestColorDistance = abs(fc1.h - fc2.h);

    if (fc1.h < fc2.h) {
        // if (longestColorDistance < 180) {
        //     newHue = fc1.h + ((manhattanDistance/((double) rad)) * (360 - longestColorDistance));
        // } else {
            newHue = fc1.h + ((manhattanDistance/((double) rad)) * longestColorDistance);
        // }
    } else {
        // if (longestColorDistance < 180) {
            newHue = fc1.h + ((manhattanDistance/((double) rad)) * (360 - longestColorDistance)) - 360 ;
        // } else {
        //     newHue = fc1.h + ((manhattanDistance/((double) rad)) * longestColorDistance);
        // }
...
// {
//     HSLAPixel color;
//     PNGmap::iterator stored = pngm.find(pair<int, int>(x, y));
//     if (stored == pngm.end())
//     {
//         double dist = abs(ctrX - x) + abs(ctrY - y);
//         color = newHue(dist);
//         pngm[pair<int, int>(x, y)] = color;
//         return color;
//     }
//     else
//     {
...
// HSLAPixel gradientColorPicker::newHue(double dist)
// {
//     HSLAPixel color;
//     double hue;
//     double newDist = (double)abs(col1.h - col2.h);
//     //case: dist greater than radius, stick to color 2
//     if (dist >= rad)
//     {
//         return col2;
//     }

//     //case: hue of col1 > hue of col2, need to go clockwise on wheel
//     if (col1.h > col2.h)
//     {
//         hue = col1.h + ((dist / ((double)rad)) * (360 - newDist)) - 360;
//         //case: hue of col2 >= hue of col1, need to go counterclockwise on wheel
//     }
//     else
//     {
//         hue = col1.h + ((dist / ((double)rad)) * newDist);
//         // if (hue > 360)
//         // {
//         //     hue = hue - 360;
//         // }
...
// HSLAPixel gradientColorPicker::setColor(double hue)
// {
//     HSLAPixel ret;
//     ret.h = hue;
//     ret.s = 1.0;
